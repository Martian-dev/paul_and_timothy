  ...
  head: () => ({
    meta: [
      { title: "Upcoming Events — Paul & Timothy Training Centre" },
      {
        name: "description",
        content:
          "Browse upcoming conferences, workshops, retreats and prayer gatherings that equip believers to discover and fulfil God's calling.",
      },
    ],
  }),
      ...
      <div className="relative mx-auto flex min-h-[92vh] max-w-7xl flex-col justify-center px-6 pt-32 pb-24 text-white">
        <motion.div initial="hidden" animate="show" variants={fadeUp}>
          <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-1.5 text-xs font-medium backdrop-blur-md">
            <CalendarDays className="h-3.5 w-3.5 text-[oklch(0.82_0.14_180)]" /> Upcoming Events · 2026 Season
          </span>
          ...
          <p className="mt-6 max-w-2xl text-base leading-relaxed text-white/80 md:text-lg">
            Join conferences, workshops, leadership training and prayer gatherings designed to help
            you discover and fulfil God's calling.
          </p>
    ...
    <Section id="featured">
      ...
      >
        ...
        <div className="p-8 md:p-10">
          ...
          <h3 className="mt-5 font-serif text-2xl font-bold text-primary md:text-3xl">{featuredEvent.title}</h3>
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{featuredEvent.description}</p>
          ...
          <div className="mt-7 grid grid-cols-4 gap-2">
            {units.map((u) => (
              <div key={u.label} className="rounded-2xl gradient-brand px-2 py-3 text-center text-primary-foreground">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a href="#events-grid" className="group inline-flex items-center gap-2 rounded-full bg-primary px-7 py-3.5 text-sm font-semibold text-primary-foreground transition-all duration-300 hover:-translate-y-0.5 hover:shadow-soft">
              Register Now <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </a>
            <a href="#events-grid" className="inline-flex items-center gap-2 rounded-full border border-border px-7 py-3.5 text-sm font-semibold text-primary transition-all duration-300 hover:-translate-y-0.5 hover:bg-accent">
              View Details
            </a>
          </div>
        </div>
...
const filterOptions = ["All", "Conference", "Workshop", "Retreat", "Prayer", "Youth", "Leadership", "Online", "Offline"];
const monthOptions = ["All Months", "September", "October", "November", "December"];
...
function UpcomingGrid() {
  ...
  const upcomingEvents = events.filter((e) => e.when === "Upcoming");
  ...
  }, [filter, month, query, upcomingEvents]);

  return (
    <Section id="events-grid" className="bg-cream">
          ...
          >
            ...
            <div className="flex flex-1 flex-col p-6">
              ...
              <div className="mt-6 flex flex-wrap gap-2 pt-2">
                <a href="#featured" className="inline-flex items-center gap-1.5 rounded-full border border-border px-5 py-2.5 text-xs font-semibold text-primary transition-colors hover:bg-accent">
                  Learn More
                </a>
                <a href="#newsletter" className="inline-flex items-center gap-1.5 rounded-full bg-primary px-5 py-2.5 text-xs font-semibold text-primary-foreground transition-transform hover:-translate-y-0.5">
      ...
      {list.length === 0 && (
        <p className="mt-12 text-center text-sm text-muted-foreground">
          No upcoming events match those filters — try another category or month.
        </p>
      )}
    </Section>
  );
...
function WhyAttend() {
  return (
    <Section className="relative overflow-hidden bg-cream">
      ...
      <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }} variants={fadeUp} className="relative text-center">
  ...
  return (
    <Section className="bg-cream">
      <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }} variants={fadeUp} className="text-center">
        ...
        <h2 className="mt-5 font-serif text-3xl font-bold text-primary md:text-4xl">Four simple steps</h2>
      </motion.div>
  ...
  return (
    <Section id="newsletter" className="bg-cream">
      ...
      >
        ...
        <h2 className="mt-5 font-serif text-3xl font-bold text-primary md:text-4xl">Never Miss an Event</h2>
        <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground">
          Receive updates about conferences, workshops, leadership training, and ministry opportunities.
        </p>
        <form onSubmit={(e) => e.preventDefault()} className="mx-auto mt-8 flex max-w-md flex-col gap-3 sm:flex-row">
          ...
          />
          <button type="submit" className="rounded-full bg-primary px-7 py-3.5 text-sm font-semibold text-primary-foreground transition-transform hover:-translate-y-0.5">
            Subscribe
          </button>
        </form>
      </motion.div>
    ...
    <Section>
      ...
      >
        ...
        <div className="relative">
          <h2 className="font-serif text-3xl font-bold md:text-5xl">Ready to Grow in Your Calling?</h2>
          <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-white/80 md:text-base">
            Join believers from around the world as we learn, serve, and build God's Kingdom together.
          </p>
          <div className="mt-9 flex flex-wrap justify-center gap-4">
            <a
              ...
              className="group inline-flex items-center gap-2 rounded-full bg-white px-7 py-4 text-sm font-semibold text-primary transition-all duration-300 hover:-translate-y-0.5 hover:shadow-soft"
            >
              Browse Upcoming Events <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </a>
            <Link
              to="/assessment"
              className="inline-flex items-center gap-2 rounded-full border border-white/30 px-7 py-4 text-sm font-semibold text-white backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:bg-white/10"
